...
public class SvgTileViewerApp8 {
    ...
    List<CustomImageTile> tiles = new ArrayList<>();
...
//  svgCanvas zum Anzeigen der SVG-Datei
	...
	private void loadFolderFiles(File folder) {
	    SwingUtilities.invokeLater(() -> {
	        ...
	        SVGDataManager dataManager = new SVGDataManager();
	        LinkedList<LinkedList<String>> svgData = dataManager.getSVGData(folder);
	        svgs.clear();
	        svgs.addAll(svgData);
	        tiles.clear();
	        ...
	        JPanel svgPanel = new JPanel(null);
	        svgPanel.setPreferredSize(new Dimension(2000, 2000)); // groß genug für Drag

	        for (LinkedList<String> data : svgData) {
	            CustomImageTile tile = new CustomImageTile(data, folder);
	            svgPanel.add(tile.getPanel());
	            tiles.add(tile);
	        }

	        // Darstellung der Tiles in ScrollPane
